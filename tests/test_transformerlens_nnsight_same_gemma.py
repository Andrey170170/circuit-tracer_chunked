...
from circuit_tracer.tracing import AttributionProblem, TraceRequest, trace_one
...
def _trace_graph(prompt, model):
    return trace_one(TraceRequest(problem=AttributionProblem(prompt=prompt, model=model))).graph
...
def models():
    ...
    model_nnsight = ReplacementModel.from_pretrained(
        "google/gemma-2-2b", "mntss/gemma-scope-transcoders", backend="nnsight"
    )
    model_tl = ReplacementModel.from_pretrained(
        "google/gemma-2-2b", "mntss/gemma-scope-transcoders"
    )
...
@pytest.fixture
def test_string_generate():
    """Test string for generation tests."""
    return "Fait: Michael Jordan joue au"
...
def test_attribution_context_consistency(models, test_string):
    ...
    assert torch.allclose(ctx_nnsight.decoder_vecs, ctx_tl.decoder_vecs, atol=5e-4, rtol=1e-5), (
...
def test_attribution_graph_consistency(models, test_string):
    ...
    with model_nnsight.zero_softcap():
        graph_nnsight = _trace_graph(test_string, model_nnsight)
    with model_tl.zero_softcap():
        graph_tl = _trace_graph(test_string, model_tl)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_unconstrained_feature_intervention_consistency(models, test_string):
    """Test that feature interventions produce consistent results."""
    model_nnsight, model_tl = models
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_unconstrained_generation_intervention_consistency(models, test_string_generate):
    """Test that feature interventions produce consistent results."""
    model_nnsight, model_tl = models

    # Perform interventions
    with model_nnsight.zero_softcap():
        intervened_str_nnsight, intervened_logits_nnsight, intervened_acts_nnsight = (
            model_nnsight.feature_intervention_generate(
                test_string_generate,
                [(20, slice(6, None), 1454, 0), (20, slice(6, None), 341, 272)],
                apply_activation_function=False,
                constrained_layers=None,
                do_sample=False,
                max_new_tokens=10,
    ...
    with model_tl.zero_softcap():
        intervened_str_tl, intervened_logits_tl, intervened_acts_tl = (
            model_tl.feature_intervention_generate(
                test_string_generate,
                [(20, slice(6, None), 1454, 0), (20, slice(6, None), 341, 272)],
                apply_activation_function=False,
                constrained_layers=None,
    ...
    if (
        intervened_str_nnsight[: len(model_nnsight.tokenizer.bos_token)]
        == model_nnsight.tokenizer.bos_token
    ):
        intervened_str_nnsight = intervened_str_nnsight[len(model_nnsight.tokenizer.bos_token) :]

    # Check tokens are same
    assert intervened_str_nnsight == intervened_str_tl, (
        f"Intervened tokens differ! NNSight: {intervened_str_nnsight}; TL: {intervened_str_tl}"
    )
...
def run_all_tests():
    ...
    model_tl = ReplacementModel.from_pretrained("google/gemma-2-2b", "gemma")
    print(model_nnsight.dtype, model_tl.cfg.dtype)
    models_fixture = (model_nnsight, model_tl)

    # Create test string directly instead of calling fixture
    test_string = "The National Digital Analytics Group (ND"

    print("Running test_get_activations_consistency...")
    test_get_activations_consistency(models_fixture, test_string)
    print("✓ Activations consistency test passed")

    print("Running test_attribution_context_consistency...")
    test_attribution_context_consistency(models_fixture, test_string)
    print("✓ Attribution context consistency test passed")

    print("Running test_attribution_graph_consistency...")
    test_attribution_graph_consistency(models_fixture, test_string)
    print("✓ Attribution graph consistency test passed")

    print("Running test_feature_intervention_consistency...")
    test_feature_intervention_consistency(models_fixture, test_string)
    print("✓ Feature intervention consistency test passed")

    print("Running test_unconstrained_feature_intervention_consistency...")
    test_unconstrained_feature_intervention_consistency(models_fixture, test_string)
    print("✓ Unconstrained feature intervention consistency test passed")

    print("Running test_unconstrained_generation_intervention_consistency...")
    test_unconstrained_generation_intervention_consistency(models_fixture, test_string)
    print("✓ Unconstrained generation intervention consistency test passed")
