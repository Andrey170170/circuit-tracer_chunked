...
def run_phase2(*, inputs: Phase2Inputs, config: Phase2Config) -> Phase2Result:
    ...
    target_selection = select_attribution_targets(
        logger=inputs.logger, model=inputs.model, ctx=inputs.ctx,
        policy=config.targets, attribution_targets=inputs.attribution_targets,
        target_logits_override=inputs.target_logits_override,
    )
    phase0 = apply_phase0_replay(
        model=inputs.model, ctx=inputs.ctx, input_ids=inputs.input_ids,
        host_activation_matrix=inputs.activation_matrix, targets=target_selection,
        observer=inputs.telemetry_observer, policy=config.phase0_replay,
    )
    record_target_replay_evidence(
        model=inputs.model, ctx=inputs.ctx, targets=target_selection,
        phase0_replay_metadata=phase0.metadata, observer=inputs.telemetry_observer,
    ...
    feature_plan = plan_active_feature_storage(
        logger=inputs.logger, model=inputs.model,
        activation_matrix=phase0.activation_matrix,
        n_logits=len(target_selection.targets), offload_handles=inputs.offload_handles,
        frontier=config.frontier, execution=config.execution,
    )
    storage = open_row_storage(
        ctx=inputs.ctx, model=inputs.model, plan=feature_plan,
        layout=config.storage_layout, runtime=config.storage_runtime,
        execution=config.execution, observer=inputs.telemetry_observer,
        offload_handles=inputs.offload_handles, owner=inputs.resource_owner,
    )
    record_storage_evidence(
        phase2_start=phase2_start, model=inputs.model, ctx=inputs.ctx,
        plan=feature_plan, storage=storage, layout=config.storage_layout,
        runtime=config.storage_runtime, execution=config.execution,
        phase0_metadata=phase0.metadata, observer=inputs.telemetry_observer,
        debug_summary=inputs.cross_cluster_debug_summary,
        debug_checkpoints=inputs.cross_cluster_debug_checkpoints,
    )
    phase3 = load_phase3_replay(
        ctx=inputs.ctx, targets=target_selection,
        activation_matrix=phase0.activation_matrix,
        n_layers=feature_plan.n_layers, n_pos=feature_plan.n_pos,
        trace_batch_size=config.execution.trace_batch_size,
        policy=config.phase3_replay, validation_context=phase0.validation_context,
    )
    return Phase2Result(
        targets=target_selection.targets, activation_matrix=phase0.activation_matrix,
        feat_layers=feature_plan.feat_layers, feat_pos=feature_plan.feat_pos,
        feat_ids=feature_plan.feat_ids, n_layers=feature_plan.n_layers,
        n_pos=feature_plan.n_pos, total_active_feats=feature_plan.total_active_feats,
        logit_offset=feature_plan.logit_offset, n_logits=feature_plan.n_logits,
        ...
        edge_matrix=storage.edge_matrix, row_to_node_index=storage.row_to_node_index,
