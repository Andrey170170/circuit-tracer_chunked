...
class _GpuResidentFeatureRowStore:
    ...
    Ordinary signed reads preserve the backing store's device contract. Exact
    Phase-4 reads use a full-residency absolute-value host mirror populated once
    from the same canonical append, avoiding repeated file reads, fine-grained
    CUDA-to-host transfers, and repeated absolute-value passes without moving
        ...
        if reason == "admitted":
            try:
                ...
                self._diagnostic_stats[
                    ...
                    self._prepared_host_rows.numel()
                    * self._prepared_host_rows.element_size()
                )
    ...
    def _range_is_resident(self, row_start: int, row_end: int) -> bool:
        return (
            ...
            and self._prepared_host_rows is not None
    ...
    def append_rows(
        ...
        **kwargs,
        ...
        try:
            ...
            canonical_host_rows = feature_rows.detach().to(device="cpu", dtype=self._dtype)
            torch.abs(
                canonical_host_rows,
        ...
        *,
        ...
        if phase == "phase4" and self._range_is_resident(row_start, row_end):
            ...
            self._record_resident_read(
                row_count,
                source_device=self._resident_rows.device,
